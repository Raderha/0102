...
export default function AiInterview() {
  ...
  const handlePublicSubmit = async (publicValue) => {
    ...
    try {
      ...
      formData.append('is_public', publicValue.toString());
  ...
  return (
    <div style={styles.container}>
      ...
      {/* 공개 여부 확인 모달 */}
      {showPublicModal && (
        <div style={styles.publicModal}>
          <div style={styles.publicModalContent}>
            <div style={styles.publicModalHeader}>
              <h3 style={styles.publicModalTitle}>공개 게시판 공개 확인</h3>
              <button
                onClick={() => {
                  setShowPublicModal(false);
                  setIsPublic(0);
                }}
                style={styles.publicModalClose}
                aria-label="닫기"
              >
                ×
              </button>
            </div>
            <div style={styles.publicModalBody}>
              <p style={styles.publicModalText}>
                공개 게시판에 질문과 답변 내용을 공개하시겠습니까?
              </p>
              <p style={styles.publicModalSubtext}>
                (음성은 텍스트로 변경되며 사용자 정보가 따로 노출되지 않습니다)
              </p>
            </div>
            <div style={styles.publicModalActions}>
              <button
                onClick={() => handlePublicSubmit(1)}
                style={styles.publicModalButtonConfirm}
              >
                확인
              </button>
              <button
                onClick={() => handlePublicSubmit(0)}
                style={styles.publicModalButtonCancel}
              >
                아니오
              </button>
            </div>
